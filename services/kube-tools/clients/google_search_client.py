...
class GoogleSearchClient:
    ...
    async def search(
        ...
        cache_key = f'google_search:{query}:{num_results}:{site_restrict or ""}'
    ...
    async def search_market_conditions(self) -> List[Dict]:
        """
        ...
        for query in queries:
            ...
            try:
                result = await self.search(
                    query=query,
                    num_results=5
                    # site_restrict='marketwatch.com OR cnbc.com OR bloomberg.com'
                )
    ...
    async def search_sector_analysis(self, sector: str) -> List[Dict]:
        """
        ...
        async def summarize_chunk(chunk):
            ...
            for i, article in enumerate(chunk, 1):
                ...
                if article.get('content'):
                    prompt_lines.append(f"   Content: {article.get('content')[:300]}")
                prompt_lines.append("")
            prompt = "\n".join(prompt_lines)
    ...
    async def fetch_article_content(self, url: str) -> Optional[str]:
        ...
        if cached_content:
            logger.info(f'Cache hit for article content: {cache_key}')
